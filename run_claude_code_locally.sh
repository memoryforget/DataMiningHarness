...
DEFAULT_CLAUDE_CONFIG_DIR="$WORKSPACE_ROOT/.claude"
...
TMP_ROOT="$WORKSPACE_ROOT/tmp/claude-code-local-batch_$RUN_STAMP"
...
if [[ ! -d "$CLAUDE_CONFIG_DIR" ]]; then
  echo "Claude config directory does not exist: $CLAUDE_CONFIG_DIR" >&2
  exit 1
fi
if [[ ! -f "$CLAUDE_CONFIG_DIR/settings.json" ]]; then
  echo "Claude settings not found: $CLAUDE_CONFIG_DIR/settings.json" >&2
  exit 1
fi
...
run_query_attempt() {
  ...
  (
    ...
    if ! command -v "$CLAUDE_BIN" >/dev/null 2>&1; then
      echo "claude is not available after environment activation" >&2
      exit 127
    fi
    {
      ...
      echo "claude=$(command -v "$CLAUDE_BIN" || true)"
      echo "conda_env=${CONDA_DEFAULT_ENV:-}"
      echo "claude_config_dir=$CLAUDE_CONFIG_DIR"
      echo "claude_settings=$CLAUDE_CONFIG_DIR/settings.json"
