...
RUN_OUTPUT_ROOT="$WORKSPACE_ROOT/tmp/benchmark_runs"
EVAL_OUTPUT_ROOT="$WORKSPACE_ROOT/tmp/benchmark_evals"
LIMIT_BENCHMARKS=""
LIMIT_TASKS=""
SKIP_EXISTING=0
JOBS=1
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --codex-home) CODEX_HOME_ARG="${2:?missing value for --codex-home}"; shift 2 ;;
...
build_generator_args() {
  ...
  if [[ -n "$CODEX_HOME_ARG" ]]; then
    args+=(--codex-home "$CODEX_HOME_ARG")
  fi
...
run_one_benchmark() {
  ...
  echo "[$domain] generating candidate reports" >&2
  mapfile -t generator_args < <(build_generator_args)
  "$HARNESS_DIR/run_codex_locally.sh" \
