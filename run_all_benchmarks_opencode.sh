...
RUN_OUTPUT_ROOT="$WORKSPACE_ROOT/tmp/benchmark_runs_opencode"
EVAL_OUTPUT_ROOT="$WORKSPACE_ROOT/tmp/benchmark_evals_opencode"
LIMIT_BENCHMARKS=""
LIMIT_TASKS=""
SKIP_EXISTING=0
JOBS=1
...
build_generator_args() {
  ...
  if [[ -n "$OPENCODE_CONFIG_DIR" ]]; then
    args+=(--opencode-config-dir "$OPENCODE_CONFIG_DIR")
  fi
  if [[ ${#OPENCODE_EXTRA_ARGS[@]} -gt 0 ]]; then
    local extra_arg
    for extra_arg in "${OPENCODE_EXTRA_ARGS[@]}"; do
      args+=(--opencode-extra-arg "$extra_arg")
    done
  fi
...
run_one_benchmark() {
  ...
  echo "[$domain] generating candidate reports with OpenCode" >&2
  mapfile -t generator_args < <(build_generator_args)
  "$HARNESS_DIR/run_opencode_locally.sh" \
